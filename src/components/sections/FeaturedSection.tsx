...
import { useGetAllProductsQuery, useAddToCartApiMutation } from "../../store/api/userApi";
...
export const FeaturedSection = () => {
  ...
  // APIs
  const { data: responseData, isLoading: isProductsLoading } = useGetAllProductsQuery("");
  const [addToCartDb, { isLoading: isAdding }] = useAddToCartApiMutation();

  const rawProducts = responseData?.data || [];

  // Smart Extraction: Grab products with badges first, fallback to newest. Limit to 2.
  const featuredProducts = rawProducts
    .filter((p: any) => p.badge) // Priority to badged items
    .concat(rawProducts) // Fallback to all items
    .filter((v: any, i: number, a: any) => a.findIndex((t: any) => (t._id === v._id)) === i) // Remove duplicates
    .slice(0, 2); // Taking top 2 for the perfect alternating layout
  ...
  if (isProductsLoading) {
    return (
      <section className="w-full bg-white py-32 flex justify-center items-center border-t border-gray-100">
      ...
      <div className="max-w-[1600px] mx-auto px-6 md:px-12">
        ...
        <div className="flex flex-col space-y-32">
          {featuredProducts.map((product: any, index: number) => {
            const isReverse = index % 2 !== 0; // Alternates automatically
            const imageUrl = product.images && product.images.length > 0 
              ? (product.images[0].url || product.images[0]) 
              : "/placeholder-image.png";
            
            // Handle notes safely
            let scentNotes = ["Premium", "Luxury"];
            if (Array.isArray(product.scentNotes)) scentNotes = product.scentNotes;
            else if (typeof product.notes === 'string') scentNotes = product.notes.split(',');
            ...
            return (
              ...
              >
                ...
                <div className="w-full lg:w-1/2 flex flex-col justify-center">
                  ...
                  >
                    {product.description || product.tagline || "A seductive, passionate composition with rich notes and deep oriental undertones."}
                  </motion.p>
                  ...
                  >
                    ...
                    <div className="flex flex-wrap gap-3">
                      {scentNotes.map((note: string, idx: number) => (
                        <span key={idx} className="px-4 py-2 border border-gray-200 text-xs font-sans text-gray-600 tracking-wider hover:border-gray-900 transition-colors">
                  ...
                  >
                    <button 
                      ...
                      className="flex items-center justify-center gap-3 bg-[#111] text-white px-8 py-4 text-[10px] font-bold uppercase tracking-[0.2em] hover:bg-[#222] transition-colors w-full sm:w-auto disabled:opacity-50"
                    >
                  ...
                  {product.stock > 0 && product.stock <= 5 && (
                    ...
                    >
                      ...
                      <span className="text-[10px] uppercase tracking-widest text-gray-500 font-bold">Only {product.stock} Left!</span>
                    </motion.div>
                  )}
