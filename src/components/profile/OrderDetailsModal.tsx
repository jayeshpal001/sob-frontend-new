...
export const OrderDetailsModal = ({
  ...
  // Standard background scroll lock
  useEffect(() => {
    if (orderId) {
                  ...
                  <div className="bg-white border border-gray-100 p-6">
                    ...
                    {isCancelled ? (
                      ...
                      <div className="relative flex justify-between items-start w-full max-w-md mx-auto">
                        <div className="absolute top-4 left-0 w-full h-1 bg-gray-100 -translate-y-1/2 z-0"></div>
                        <div
                          className="absolute top-4 left-0 h-1 bg-green-500 -translate-y-1/2 z-0 transition-all duration-500"
                          style={{
                        ...
                        {trackingSteps.map((step, index) => {
                          ...
                          return (
                            ...
                            >
                              <div
                                className={`w-8 h-8 rounded-full flex items-center justify-center border-2 transition-colors ${isCompleted ? "bg-green-500 border-green-500 text-white" : "bg-white border-gray-200 text-gray-300"}`}
                              >
                                {isCompleted ? (
                              ...
                              <span
                                className={`text-[9px] uppercase tracking-widest font-bold ${isActive ? "text-gray-900" : isCompleted ? "text-green-600" : "text-gray-400"}`}
                              >
                                {step}
                              </span>
                            </div>
                    ...
                    <div className="bg-gray-50 p-5 border border-gray-100">
                      ...
                      <div className="space-y-2 text-xs font-sans">
                        ...
                        {order.couponCode && order.discountAmount > 0 && (
                          <div className="flex justify-between font-medium text-green-600">
                            <span className="flex items-center gap-1">
                              <Ticket className="w-3 h-3" /> Discount (
                              {order.couponCode})
                            </span>
                            <span>
                        ...
                        <div className="pt-2 mt-2 border-t border-gray-200 flex justify-between items-center">
                          ...
                          <span className="text-[9px] font-bold uppercase tracking-widest px-2 py-1 bg-white border border-gray-200">
                            {order.paymentMethod}
                          </span>
                        </div>
                      </div>
