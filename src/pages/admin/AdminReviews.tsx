...
export const AdminReviews = () => {
  ...
  const { data: responseData, isLoading, isError } = useGetAllAdminReviewsQuery();
  ...
  const handleApprove = async (reviewId: string) => {
    try {
      await approveReview({ id: reviewId, status: "approved" }).unwrap();
  ...
  const handleReject = async (reviewId: string) => {
    try {
      await rejectReview({ id: reviewId, status: "rejected" }).unwrap();
